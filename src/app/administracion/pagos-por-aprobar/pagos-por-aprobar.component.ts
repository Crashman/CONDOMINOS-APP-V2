...
@Component({
  selector: 'app-pagos-por-aprobar',
  templateUrl: './pagos-por-aprobar.component.html',
  styleUrls: ['./pagos-por-aprobar.component.css']
})
export class PagosPorAprobarComponent implements OnInit {
  ...
  /*Pendiente de aprobación = Creado*/
  getPagos() {
    this.pagoService.getPagosPorCondominio(this._codigoCondominio,0).subscribe(data => {
  ...
  rejectPago(codigoPago: number, codigoRequerimiento: number) {
    if (confirm('Desea rechazar el pago?') === true) {
      this.pagoService
        .rejectPago(codigoRequerimiento, codigoPago)
        .subscribe(data => {
