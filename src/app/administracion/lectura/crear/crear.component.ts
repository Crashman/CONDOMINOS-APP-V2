...
export class CrearComponent implements OnInit {
  ...
  newLectura(content1: string) {
    ...
    this.lecturaService.newLectura(this.codigoPropiedad).subscribe(data => {
      for (const x of data) {
        if  (x.codigoLectura === 0) {
          //this.showNotification('error', x.fechaStr.toString() );
        } else {
  ...
  gotoHome(){
    this.router.navigate(['CAPP/administracion/lectura']);  // define your component where you want to go
  }
